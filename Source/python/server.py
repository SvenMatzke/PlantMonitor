...
def run_server(buf, restful_online_time, keep_alive_time, loaded_settings):
    ...
    # declare pages
    plant_app = userv.socketserver.App()

    def _index(writer, request):
        global last_request_time
        last_request_time = time.time()
        return userv.socketserver.static_file(writer, "index.html", buf)

    def _static_js(writer, request):
        return userv.socketserver.static_file(writer, "app.bundle.js", buf)
    ...
    def _get_data(writer, request):
        ...
        return userv.socketserver.json(writer, sens_data)
    ...
    def _sensor_configure(writer, request):
        ...
        return userv.socketserver.json(writer, configuration_data)
    ...
    def _get_settings(writer, request):
        global last_request_time
        last_request_time = time.time()
        return userv.socketserver.json(writer, settings.get_settings())
    ...
    def _post_settings(writer, request):
        ...
        try:
            ...
            return userv.socketserver.json(writer, {"message": "Request had no valid json body."}, status=406)
